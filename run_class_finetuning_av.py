...
def get_args():
    ...
    # Model parameters
    parser.add_argument('--model', default='avit_dim512_patch16_160_a256', type=str, metavar='MODEL',
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/content/drive/MyDrive/HiCMAE/saved/data/crema-d/audio_visual/split01', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--data_set', default='CREMA-D', choices=['Kinetics-400', 'SSV2', 'UCF101', 'HMDB51','image_folder',
                        'DFEW', 'MAFW', 'RAVDESS', 'CREMA-D', 'AVCAFFE', "MER2023", "MSP-IMPROV", "IEMOCAP"],
...
def main(args, ds_init):
    ...
    print("VALUE OF DATA SET TRAIN")
    print(data_loader_train)
    #print(f'data {dataset_train} sampler {sampler_train}')
    ...
    if global_rank == 0:
        ...
        final_top1_best, final_top5_best, pred_dict_best = merge(args.output_dir, num_tasks, args, best=True)
        print(f"Accuracy of the network on the {len(dataset_test)} test videos using best epoch model: Top-1: {final_top1_best:.2f}%, Top-5: {final_top5_best:.2f}%")
        ...
        conf_mat_best = confusion_matrix(y_pred=preds_best, y_true=labels_best)
        print(f'Confusion Matrix:\n{conf_mat_best}')
        ...
        weighted_f1_best = f1_score(y_pred=preds_best, y_true=labels_best, average='weighted')
        micro_f1_best = f1_score(y_pred=preds_best, y_true=labels_best, average='micro')
        macro_f1_best = f1_score(y_pred=preds_best, y_true=labels_best, average='macro')
        print(f'Weighted F1: {weighted_f1_best:.4f}, micro F1: {micro_f1_best:.4f}, macro F1: {macro_f1_best:.4f}')
        ...
        if args.output_dir and utils.is_main_process():
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                ...
                f.write(f'Final Confusion Matrix:\n{conf_mat_best}\n')
                f.write(f'Final Class Accuracies: {[f"{i:.2%}" for i in class_acc_best]}\n')
